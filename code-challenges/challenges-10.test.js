...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let arr1 = [];
  input.forEach(element =>{
    arr = element.filter(val => val%5 === 0 && typeof val === 'number');
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let newArr = data.filter(names => {
    if(names.gender==='female' || names.gender==='male' )
    {
      return names.name;
    }
  });
  let sonArr = newArr.map(element=>
  {
    return element.name;
  });
  let result = sonArr.join( ' and ' );
...
let findShortest = (data) => {
  // Solution code here...
  let arr1 = data.filter(element => {
    if(element.gender!=='female' & element.gender!=='male' & element.name !=='C-3PO')
    {
      return element.name;
    }
  });
